...
export default async function Navbar({ className, ...props }: React.HTMLAttributes<HTMLElement>) {
  ...
  const navigationLinks = [
    ...
    { href: isPatientUser ? "/journal" : "/my-patients",
      text: isPatientUser ? "Journal" : "My patients" },
    { href: isPatientUser ? "/my-plan" : "/records",
      text: isPatientUser ? "My Plan" : "Records" },
    { href: "/messages", text: "Messages" },
  ];

  return (
    <nav className={cn("flex flex-col space-y-4 lg:space-y-6 pl-10", className)} {...props}>
